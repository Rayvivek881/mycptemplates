...
/*
  ...
  range sum in log(n);
  upper_bond and lowerbond in log(n) if array is sorted;
  shift(range increament with val) in log(n);
  get_sum (range sum) in log(n);
  insert at anypoint in log(n);
  erase at anypoint in log(n);
  reverse any subarray in log(n)
*/
template <typename T>
class treap {
 private: // https://gitgud.io/Xellos/treapset/-/blob/master/treap.hpp
  struct node;
...
int main(int argc, char const *argv[])
{
    ...
    treap<int> Tree;
    for (int i = 0; i < 8; i++) Tree.insert(lst[i]);
    Tree.reverse(1, 5);
    for (int i = 0; i < 8; i++) cout << Tree[i] << ' ';
    cout << endl << Tree.get_sum(0, 5);
    return 0; 
}
